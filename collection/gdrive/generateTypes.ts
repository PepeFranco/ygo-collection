...
const generateTypes = () => {
  try {
    ...
    console.log(
      "✅ Successfully generated collection.types.ts from headers.json"
    );
    console.log(
      `📄 Generated ${
        headersConfig.headers.filter((h) => h && h.trim() !== "").length
      } fields for CollectionRow`
    );
  } catch (error) {
