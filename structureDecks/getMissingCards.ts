...
export const getMissingCards = () => {
  ...
  // Find exact cards
  const result: { deck: string; cardsMissing: string[] }[] = [];
  orderedCardSets.map((cardSet) => {
    ...
    const setResult: { deck: string; cardsMissing: string[] } = { deck: cardSet.set_name, cardsMissing: [] };
